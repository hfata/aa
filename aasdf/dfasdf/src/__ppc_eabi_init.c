...
extern unsigned char __ram_code_start[];
extern unsigned char __ram_code_end[];
extern unsigned char __ram_code_load[];
...
static void InitRamCode(void)
{
	unsigned char *dst = __ram_code_start;
	unsigned char *src = __ram_code_load;
	unsigned int size = (unsigned int)(__ram_code_end - __ram_code_start);
	unsigned int i;
	...
	/*
	 * Shadow flash erase/program must branch into SRAM after this copy.
	 * Force memory visibility and discard any stale prefetched instructions
	 * before the first call into .ram_code.
	 */
	asm {
		msync
		isync
